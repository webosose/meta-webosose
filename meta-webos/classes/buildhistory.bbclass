...
#
# WEBOS - Added SRC_URI to the items written to the buildhistory
#       - Collecting information during the fetch do produce a BOM
#       - Measure time spent in various functions
#       - buildhistory.bbclass taken from branch=master,commit=6db6c74 of oe-core
#
...
#WEBOS Pulled function out to make it available elsewhere
    ...
    if e.data.getVar('BUILDHISTORY_FEATURES', True).strip():
        ...
        if isinstance(e, bb.event.BuildStarted):
            ...
            if e.data.getVar("BUILDHISTORY_COMMIT", True) == "1":
                ...
                # WEBOS: measure time spent in buildhistory_commit function
                import time
                start=time.time()
...
python write_srcrev() {
    # WEBOS: Collect any data we can think of about this component
    #        It needs to be done during or after do_fetch as that's
    #        the only thing we seem to do for everything.
    def _webos_append_to_bom(d, srcrevs):
        data = {}

        data['pn'] = d.getVar('PN', True)
        data['pe'] = d.getVar('PE', True) or "0"
        data['pv'] = d.getVar('PV', True)
        data['pr'] = d.getVar('PR', True)
        data['depends'] = sortlist(oe.utils.squashspaces(d.getVar('DEPENDS', True) or ""))
        data['bbfile'] = d.getVar('BB_FILENAME', True)
        data['src_uri'] = d.getVar('SRC_URI', True)
        data['srcrev'] = srcrevs

        packages = oe.utils.squashspaces(d.getVar('PACKAGES', True))
    ...
    if srcrevs:
        ...
        with open(srcrevfile, 'w') as f:
            ...
            orig_srcuri = d.getVar('SRC_URI', True) or 'INVALID' # WEBOS
            f.write('SRC_URI = "%s"\n' % orig_srcuri)            # WEBOS
            ...
            if len(srcrevs) > 1:
                for name, srcrev in srcrevs.items():
                    ...
                    if orig_srcrev:
                        f.write('# SRCREV_%s = "%s"\n' % (name, orig_srcrev))
                    f.write('SRCREV_%s = "%s"\n' % (name, srcrev))
                    bom_srcrevs.append(srcrev) # WEBOS
            else:
                f.write('SRCREV = "%s"\n' % srcrevs.values())
                bom_srcrevs.append(list(srcrevs.values())) # WEBOS
    ...
    _webos_append_to_bom(d,bom_srcrevs) # WEBOS
}
